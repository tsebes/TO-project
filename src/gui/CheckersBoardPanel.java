...
public class CheckersBoardPanel extends JPanel implements BoardObserver{
    ...
    private void movePiece(Coordinates piece, List<Coordinates> places){
        ...
        for(int i = 1; i < places.size(); i += 1){
            tiles.get(places.get(i).getY() * TILE_COUNT + places.get(i).getX()).removePiece();
        }
        for (int i = 0; i < TILE_COUNT; i++) {
            for (int j = 0; j < TILE_COUNT; j++) {
                if ((i + j) % 2 == 1) {
